...
function categoriserPoste(poste) {
  ...
  if (
    ...
    p.includes("CONTROLE FINAL") ||
    p.includes("CTRL FINAL") ||
    p.includes("CONTRÔLE FINAL")
  )
    return "Contrôle Final";
  if (
    p.includes("RECEPTION") ||
    p.includes("RÉCEPTION") ||
    p.includes("CTRL ENTREE") ||
    p.includes("CONTROLE ENTREE")
  )
    return "Contrôle à réception";
  if (
    p.includes("FABRICATION") ||
    p.includes("PRESSE") ||
    p.includes("SOUDURE") ||
    p.includes("MONTAGE") ||
  ...
  if (
    p.includes("ESSAI") ||
    p.includes("ETALONNAGE") ||
    p.includes("ÉTALONNAGE") ||
    p.includes("BANC")
  )
  ...
  if (
    ...
    return "Client";
  if (p.includes("FOURNISSEUR") || p.includes("SOUS-TRAITANT"))
...
function mapNconformiteToFE(row, site = "soucy") {
  return {
    ...
    designation: row.LibelleNonConf, // libellé court NC
    detail_anomalie: row.DetailAnomalie || "", // texte long = "Détail de l'anomalie" depuis COMMENTEXT
    designation_article: row.DesignationArticle || "",
    decouvert_par: row.DecouvertPar || row.VarAlphaUtil6 || null,
...
export async function getAllFE({
  ...
  if (origine) {
    whereClause += ` AND LTRIM(RTRIM(n.OrigineNonConf)) = @origine`;
    params.origine = origine;
  }
  if (q?.trim()) {
    whereClause += ` AND (n.NonConformite LIKE @search OR n.CodeArticle LIKE @search OR n.VarAlphaUtil LIKE @search OR n.LibelleNonConf LIKE @search OR n.ClientFourn LIKE @search)`;
    params.search = `%${q.trim()}%`;
  }
  if (statut && statut !== "Tous") {
    whereClause += ` AND n.NonConfTraite = @statut`;
    params.statut = statut === "Traitée" ? "O" : "N";
  }
  if (code_article && !q) {
    whereClause += ` AND n.CodeArticle LIKE @code_article`;
    params.code_article = `%${code_article}%`;
  }
  if (date_debut) {
    whereClause += ` AND n.DateNonConf >= @date_debut`;
    params.date_debut = date_debut;
  }
  if (date_fin) {
    whereClause += ` AND n.DateNonConf <= @date_fin`;
    params.date_fin = date_fin;
  }

  const countResult = await query(
    `SELECT COUNT(*) as total FROM dbo.NCONFORMITE n ${whereClause}`,
    params,
    site,
  );
  ...
  const dataSql = `
    SELECT n.*, a.Designation1 as DesignationArticle,
           CAST(c.Commentaire AS NVARCHAR(MAX)) as DetailAnomalie
    FROM dbo.NCONFORMITE n
    LEFT JOIN dbo.ARTICLE a    ON a.CodeArticle = n.CodeArticle
    LEFT JOIN dbo.COMMENTEXT c ON c.IdText = n.IdDescription
    ${whereClause}
    ORDER BY n.DateNonConf DESC
...
export async function getFEByNumero(numeroFE, site = "soucy") {
  const result = await query(
    `SELECT n.*, a.Designation1 as DesignationArticle,
     ...
     LEFT JOIN dbo.ARTICLE a    ON a.CodeArticle = n.CodeArticle
     LEFT JOIN dbo.COMMENTEXT c ON c.IdText = n.IdDescription
     WHERE n.NonConformite = @numero_fe`,
    { numero_fe: numeroFE },
    site,
  );
...
export async function getFEStatsByMonth(
...
export async function searchFE(searchTerm, limit = 100, site = "soucy") {
  const result = await query(
    `
    SELECT TOP (@limit) n.*, a.Designation1 as DesignationArticle
    FROM dbo.NCONFORMITE n
    LEFT JOIN dbo.ARTICLE a ON a.CodeArticle = n.CodeArticle
    WHERE n.NonConformite LIKE @search OR n.CodeArticle LIKE @search
      OR n.VarAlphaUtil LIKE @search OR n.LibelleNonConf LIKE @search OR n.ClientFourn LIKE @search
    ORDER BY n.DateNonConf DESC
  `,
...
export async function getFECountParArticle(site = "soucy") {
  const result = await query(`
    SELECT
      n.CodeArticle                                           AS code_article,
      LEFT(LTRIM(RTRIM(n.CodeArticle)), 3)                   AS client,
      MAX(a.Designation1)                                     AS designation_article,
      COUNT(*)                                                AS total,
      SUM(CASE WHEN n.NonConfTraite = 'N' THEN 1 ELSE 0 END) AS en_cours,
      SUM(CASE WHEN n.NonConfTraite = 'O' THEN 1 ELSE 0 END) AS traitees,
      MAX(n.NonConformite)                                    AS derniere_fe,
      MAX(n.DateNonConf)                                      AS date_derniere
    FROM dbo.NCONFORMITE n
    LEFT JOIN dbo.ARTICLE a ON a.CodeArticle = n.CodeArticle
    WHERE n.CodeArticle IS NOT NULL AND LTRIM(RTRIM(n.CodeArticle)) != ''
    GROUP BY n.CodeArticle
    ORDER BY total DESC
  `, {}, site);
  return result.recordset;
}
