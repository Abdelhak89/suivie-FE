...
function getDataByKeys(fe, ...keys) {
  ...
  for (const [k, v] of Object.entries(data)) { if (wanted.has(cleanKey(k))) return v ?? ""; }
  return "";
}

function parseQteEstimee(fe) {
  const v = fe.qte_non_conforme || getDataByKeys(fe, "Qte estimee","Qte estimée","Qté estimée","Qte NC","Qté NC") || "";
...
function getTypeDefaut(fe) {
  return String(fe.type_nc || getDataByKeys(fe,"Type de défaut","Type de defaut","Type défaut","Type defaut") || "Non renseigné");
}

function getIlot(fe) {
  return String(getDataByKeys(fe,"ILOT GENERATEUR","Ilot générateur","Ilot generateur") || fe.ilot || "Non renseigné");
}
...
function isoToYmd(v) {
  ...
  const d = new Date(v);
  if (Number.isFinite(d.getTime())) return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`;
  return "";
}
...
function enrichApp(fe) {
  return {
    ...fe,
    _client:     getClientNameFromRef(fe?.code_article || "") || "App",
    _ilot:       fe.ilot || "Non renseigné",
    _origine:    fe.type_evenement === "nc_fournisseur" ? "Fournisseur" : fe.type_evenement === "nc_client" ? "Client" : "Interne",
    _typeDefaut: fe.type_evenement || "Non renseigné",
    _qteNc:      Number(fe.quantite) || 0,
...
function ChartPanel({ title, sub, children, fullWidth }) {
  return (
    <div className="panel" style={fullWidth ? { gridColumn:"1 / -1" } : {}}>
      ...
      <div style={{ height:260, marginTop:8 }}>{children}</div>
    </div>
  );
}
...
export default function KpiPage() {
  ...
  const [allDiap,     setAllDiap]     = useState([]);
  ...
  useEffect(() => {
    ...
    getAllFE({ annee: annee || null, limit: 5000 })
      .then(r => { if (!ctrl.signal.aborted) setAllDiap(r.items || []); })
      .catch(() => {})
      .finally(() => { if (!ctrl.signal.aborted) setLoadingDiap(false); });
    return () => ctrl.abort();
  }, [annee]);
  ...
  const clientsList  = useMemo(() => [...new Set(all.map(x => x._client).filter(Boolean))].sort((a,b) => a.localeCompare(b,"fr")), [all]);
  const ilotsList    = useMemo(() => [...new Set(all.map(x => x._ilot).filter(Boolean))].sort((a,b) => a.localeCompare(b,"fr")), [all]);
  const originesList = useMemo(() => [...new Set(all.map(x => x._origine).filter(Boolean))].sort((a,b) => a.localeCompare(b,"fr")), [all]);
  const typesList    = useMemo(() => [...new Set(all.map(x => x._typeDefaut).filter(Boolean))].sort((a,b) => a.localeCompare(b,"fr")), [all]);
  ...
  ), [all, fClient, fIlot, fOrigine, fTypeDefaut]);
  ...
  const lineWeekly = useMemo(() => {
    const map = new Map();
    for (const x of filtered) { const key = x._date ? x._date.slice(0,7) : "NA"; map.set(key, (map.get(key)||0)+1); }
    ...
    <div className="container">
      <div className="pageHead">
        ...
        <SourceToggle
          ...
          diapasonCount={enrichedDiap.length} appCount={enrichedApp.length}
        />
      </div>

      <div className="toolbar" style={{ border:"none", background:"none", padding:0, marginBottom:8 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(2, 1fr)", gap:14 }}>
        ...
        <ChartPanel title="Pareto — Îlot générateur (Top 10)" sub="Clique une barre pour voir le détail">
          <ResponsiveBar data={paretoIlot.map(d => ({ label:d.label, FE:d.value }))} keys={["FE"]} indexBy="label"
            ...
            colors={BAR_COLORS} theme={NIVO_THEME} axisBottom={{ tickRotation:-35 }}
        ...
        <ChartPanel title="Pareto — Type de défaut (Top 10)" sub="Clique une barre pour voir le détail">
          <ResponsiveBar data={paretoType.map(d => ({ label:d.label, FE:d.value }))} keys={["FE"]} indexBy="label"
            margin={{ top:10, right:18, bottom:70, left:50 }} padding={0.25} enableLabel={false}
            colors={BAR_COLORS} theme={NIVO_THEME} axisBottom={{ tickRotation:-35 }}
        ...
        <ChartPanel title="Répartition — Interne / Client / Fournisseur" sub="Clique une part pour voir le détail">
          <ResponsivePie data={pieOrigine} margin={{ top:10, right:16, bottom:10, left:16 }}
            innerRadius={0.55} padAngle={1.2} cornerRadius={6} activeOuterRadiusOffset={8}
            enableArcLabels={false} theme={NIVO_THEME}
        ...
        <ChartPanel title="Pareto — Clients (Top 10)" sub="Client = 3 premiers chiffres de la REF">
          <ResponsiveBar data={paretoClient.map(d => ({ label:d.label, FE:d.value }))} keys={["FE"]} indexBy="label"
            margin={{ top:10, right:18, bottom:70, left:50 }} padding={0.25} enableLabel={false}
            colors={BAR_COLORS} theme={NIVO_THEME} axisBottom={{ tickRotation:-35 }}
        ...
        <ChartPanel title="Tendance — FE par mois" sub="Clique un point pour voir le détail" fullWidth>
          <div style={{ height:300 }}>
            <ResponsiveLine data={lineWeekly} margin={{ top:20, right:20, bottom:60, left:50 }}
              xScale={{ type:"point" }} yScale={{ type:"linear", min:0, max:"auto" }}
              axisBottom={{ tickRotation:-35 }} pointSize={8} useMesh colors={["#2563eb"]} theme={NIVO_THEME}
      ...
      {drill && (
        <div className="modalBackdrop" onMouseDown={() => setDrill(null)}>
          <div className="modal" onMouseDown={e => e.stopPropagation()} style={{ maxWidth:1200, padding:16, display:"flex", flexDirection:"column", maxHeight:"88vh" }}>
            ...
            <div className="tableWrap" style={{ flex:1, overflow:"auto" }}>
              <table className="table">
                <thead>
                  <tr>
                    {["N° FE","Date","Client","Îlot","Type","REF","Désignation","Qté NC","Source"].map(h => (
                ...
                <tbody>
                  {drill.rows.slice(0,300).map(x => (
                    <tr key={x.numero_fe} className="rowHover">
                      ...
                      <td className="td"><span style={{ fontSize:10, padding:"2px 7px", borderRadius:10, background:x._source==="app"?"#e8fdf0":"rgba(0,0,0,0.06)", color:x._source==="app"?"#1a7a3f":"#6e6e73", fontWeight:700 }}>{x._source==="app"?"App":"SILOG"}</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
