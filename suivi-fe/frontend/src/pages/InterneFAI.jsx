...
function is8DComplete(fe) {
  try { const p = JSON.parse(fe.analyse_8d || ""); return !!(p?.responsable_qualite && p?.date_cloture); }
  catch { return false; }
}
...
function EmplacementCell({ lct }) {
  ...
  const pills     = rawDetail ? rawDetail.split(" | ").map(s => s.trim()).filter(Boolean)
                               : principal ? [principal] : [];
  const hasPrison = pills.some(p => p.toUpperCase().includes("PRISON")) || zone.toUpperCase().includes("PRISON");
  if (!pills.length && !zone) return <span className="empl-pill unknown">NON LOCALISÉ</span>;
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
      {zone && <div style={{ fontSize: 10, fontWeight: 700, color: hasPrison ? "#ff3b30" : "#6e6e73", textTransform: "uppercase", letterSpacing: ".4px" }}>📦 {zone}</div>}
      <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
        {pills.map((pill, i) => {
          const isPrison = pill.toUpperCase().includes("PRISON");
          return <span key={i} className={`empl-pill ${isPrison ? "prison" : "normal"}`}>{isPrison ? "🔒 " : ""}{pill}</span>;
        })}
      </div>
    </div>
  );
...
async function fetchLancements(codeArticle) {
  ...
  try {
    const r = await fetch(`${API}/api/lancements?code_article=${encodeURIComponent(codeArticle)}&limit=30`);
    if (!r.ok) return [];
...
function LctStatutBadge({ lct }) {
  ...
  if (poste)   return <span className="ap-badge ap-badge-orange">En cours · {poste}</span>;
...
function LancementModal({ fe, onClose }) {
  ...
  useEffect(() => {
    fetchLancements(fe?.code_article).then(setLcts).catch(() => setLcts([])).finally(() => setLoading(false));
  }, [fe?.code_article]);
  ...
  const openMailto = () => {
    const subj = encodeURIComponent(`DVI incomplet — FE ${fe?.numero_fe || ""} — ${selLct?.CodeLancement || ""}`);
    ...
    <div className="lct-backdrop" onMouseDown={onClose}>
      <div className="lct-modal" onMouseDown={e => e.stopPropagation()}>
        <div className="lct-modal-head">
          <div>
            <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 4 }}>Lancements article — {fe?.code_article}</div>
            <div style={{ fontSize: 12, color: "#6e6e73" }}>FE <b style={{ color: "#0071e3" }}>{fe?.numero_fe}</b> · {client.name}</div>
          </div>
          <button className="ap-btn ap-btn-ghost" onClick={onClose} style={{ fontSize: 18, padding: "3px 10px" }}>✕</button>
        </div>
        <div className="lct-modal-body">
          {loading ? (
            <div style={{ padding: 32, textAlign: "center", color: "#aeaeb2" }}>Chargement…</div>
          ) : lcts.length === 0 ? (
            <div style={{ padding: 32, textAlign: "center", color: "#aeaeb2" }}>Aucun lancement trouvé</div>
          ) : (
            <>
              <div style={{ fontSize: 11.5, color: "#6e6e73", marginBottom: 12, background: "rgba(0,113,227,0.06)", borderRadius: 8, padding: "8px 12px" }}>
                ...
                <table className="fai-table">
                  ...
                  <tbody>
                    {lcts.map(lct => {
                      ...
                      return (
                        <tr key={lct.CodeLancement} className={`fai-row${sel ? " lct-row-sel" : ""}`} style={{ cursor: "pointer" }} onClick={() => setSelLct(sel ? null : lct)}>
                          <td className="fai-td" style={{ paddingLeft: 14 }}>
                            <input type="radio" readOnly checked={sel} style={{ accentColor: "#0071e3", cursor: "pointer", width: 14, height: 14 }} />
                          </td>
                          <td className="fai-td"><span style={{ fontWeight: 700, color: "#0071e3", fontSize: 13 }}>{lct.CodeLancement}</span></td>
                          <td className="fai-td"><LctStatutBadge lct={lct} /></td>
                          <td className="fai-td" style={{ fontSize: 12, color: "#6e6e73" }}>{lct.CodePosteEnCours || "—"}</td>
                          <td className="fai-td" style={{ fontSize: 12, color: "#6e6e73" }}>{lct.DateFinPlanifiee ? new Date(lct.DateFinPlanifiee).toLocaleDateString("fr-FR") : "—"}</td>
                          <td className="fai-td" style={{ fontWeight: 600 }}>{lct.QuantiteLancee != null ? Number(lct.QuantiteLancee).toLocaleString("fr-FR") : "—"}</td>
                        </tr>
                      );
                    })}
                  </tbody>
              ...
              {selLct && (
                <div>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: "#6e6e73", textTransform: "uppercase", letterSpacing: ".5px" }}>Aperçu du mail</div>
                    <button className="ap-btn ap-btn-primary" onClick={openMailto} style={{ fontSize: 12, padding: "6px 14px" }}>✉️ Ouvrir dans ma messagerie</button>
                  </div>
...
function BlocageTab() {
  ...
  useEffect(() => {
    fetch(`${API}/api/lancements?statut=termine_dvi&limit=500`)
      .then(r => r.json()).then(d => setLcts(d.data || [])).catch(() => setLcts([])).finally(() => setLoading(false));
  }, []);

  const toggleSort = key => { setPage(1); setSort(s => s.key === key ? { key, dir: s.dir === "asc" ? "desc" : "asc" } : { key, dir: "asc" }); };
  ...
  const sorted = useMemo(() => {
    ...
    arr.sort((a, b) => {
      ...
      if (key === "DateSoldeLancement" || key === "DateFinPlanifiee") { va = va ? new Date(va).getTime() : 0; vb = vb ? new Date(vb).getTime() : 0; }
      else if (key === "QuantiteLancee") { va = Number(va) || 0; vb = Number(vb) || 0; }
  ...
  const filtered = useMemo(() => {
    let base = sorted.filter(l => { const qte = Number(l.QuantiteLancee); return !isNaN(qte) && qte > 0; });
    if (!filterQ.trim()) return base;
    const q = filterQ.toLowerCase();
    return base.filter(l => (l.CodeLancement || "").toLowerCase().includes(q) || (l.CodeArticle || "").toLowerCase().includes(q) || (l.DesignationArt1 || "").toLowerCase().includes(q) || (l.EmplacementStock || "").toLowerCase().includes(q));
  }, [sorted, filterQ]);

  const totalPages = Math.max(1, Math.ceil(filtered.length / BLOC_PAGE));
  const paged      = useMemo(() => filtered.slice((page - 1) * BLOC_PAGE, page * BLOC_PAGE), [filtered, page]);
  const SortIcon   = ({ col }) => sort.key !== col ? <span style={{ opacity: 0.25, marginLeft: 4 }}>↕</span> : <span style={{ marginLeft: 4, color: "#0071e3" }}>{sort.dir === "asc" ? "↑" : "↓"}</span>;

  return (
    <div>
      <div className="fai-toolbar">
        {!loading && <span className="ap-badge ap-badge-orange">{filtered.length} résultat{filtered.length > 1 ? "s" : ""}</span>}
        <input className="ap-input" style={{ flex: 1, minWidth: 200 }} value={filterQ} onChange={e => { setFilterQ(e.target.value); setPage(1); }} placeholder="🔍  Recherche N° lancement, article, emplacement…" />
        {filterQ && <button className="ap-btn" onClick={() => { setFilterQ(""); setPage(1); }}>✕</button>}
      </div>
      {loading ? (
        ...
        <>
          <div className="fai-table-wrap">
            <table className="fai-table">
              <thead>
                <tr>{BLOC_COLS.map(col => <th key={col.key} className="fai-th" style={{ cursor: "pointer", userSelect: "none" }} onClick={() => toggleSort(col.key)}>{col.label}<SortIcon col={col.key} /></th>)}</tr>
              </thead>
              <tbody>
                {paged.map(lct => (
                  <tr key={lct.CodeLancement} className="fai-row" style={{ background: "#fffcf0" }}>
                    <td className="fai-td" style={{ fontWeight: 700, whiteSpace: "nowrap" }}>{lct.CodeLancement}</td>
                    <td className="fai-td"><EmplacementCell lct={lct} /></td>
                    <td className="fai-td" style={{ fontFamily: "monospace", fontSize: 12 }}>{lct.CodeArticle}</td>
                    <td className="fai-td" style={{ fontSize: 12, color: "#6e6e73" }}>{lct.DesignationArt1 || lct.DesignationLct1 || "—"}</td>
                    <td className="fai-td" style={{ fontSize: 12, color: "#6e6e73", whiteSpace: "nowrap" }}>{lct.DateSoldeLancement ? new Date(lct.DateSoldeLancement).toLocaleDateString("fr-FR") : "—"}</td>
                    <td className="fai-td" style={{ fontWeight: 800, textAlign: "right" }}>{Number(lct.QuantiteLancee).toLocaleString("fr-FR")}</td>
                    <td className="fai-td"><span className="ap-badge ap-badge-orange">{lct.MotifBlocage || "DVI"}</span></td>
                    <td className="fai-td" style={{ fontSize: 11.5, color: "#6e6e73", maxWidth: 200 }}>{lct.CommentaireBlocage || "—"}</td>
                  </tr>
                ))}
                {!paged.length && <tr><td className="fai-td" colSpan={BLOC_COLS.length} style={{ textAlign: "center", padding: 48, color: "#aeaeb2" }}>Aucun lancement bloqué trouvé</td></tr>}
              </tbody>
            </table>
          </div>
          <div className="ap-pagination">
            <button className="ap-btn" onClick={() => setPage(1)} disabled={page <= 1}>«</button>
            <button className="ap-btn" onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page <= 1}>← Préc.</button>
            <span style={{ color: "#6e6e73", fontSize: 13 }}>Page <b>{page}</b> / {totalPages}<span style={{ color: "#aeaeb2", marginLeft: 8 }}>({filtered.length} résultats)</span></span>
            <button className="ap-btn" onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page >= totalPages}>Suiv. →</button>
            <button className="ap-btn" onClick={() => setPage(totalPages)} disabled={page >= totalPages}>»</button>
          </div>
        </>
      )}
    </div>
...
export default function InterneFAI() {
  ...
  useEffect(() => {
    ...
    if (!document.getElementById("fai-css")) {
      const s = document.createElement("style"); s.id = "fai-css"; s.textContent = FAI_CSS;
  ...
  useEffect(() => {
    setLoadingData(true);
    getAllFE({ q: q.trim() || null, statut: statut === "Tous" ? null : statut, annee, limit: 2000 })
      .then(r => {
        const fai = (r.items || []).filter(fe => fe.is_dvi === true);
        setItems(fai);
        setAssignments(prev => {
          const next = { ...prev };
          fai.forEach(fe => { if (!next[fe.numero_fe]) next[fe.numero_fe] = { mesure_efficacite: null, cloture: fe.statut?.toLowerCase().includes("traité") || false }; });
          return next;
        });
  ...
  const setAss = (fid, field, val) => setAssignments(p => ({ ...p, [fid]: { ...p[fid], [field]: val } }));

  const totalPages = Math.max(1, Math.ceil(items.length / pageSize));
  const pagedRows  = useMemo(() => items.slice((page - 1) * pageSize, page * pageSize), [items, page]);
  ...
  const nbEffOui   = Object.values(assignments).filter(a => a.mesure_efficacite === true).length;

  // Compteurs 8D depuis KEP_NC
  const nb8DEnCours  = items.filter(fe => analyses8D[fe.numero_fe] && !is8DComplete({ analyse_8d: analyses8D[fe.numero_fe] })).length;
  const nb8DComplets = items.filter(fe => analyses8D[fe.numero_fe] &&  is8DComplete({ analyse_8d: analyses8D[fe.numero_fe] })).length;
  ...
  return (
    <div style={{ fontFamily: "-apple-system,BlinkMacSystemFont,'Helvetica Neue',sans-serif", padding: "24px 28px", background: "#f5f5f7", minHeight: "100vh" }}>
      ...
      <div className="ap-page-head">
        <div>
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 6, flexWrap: "wrap" }}>
            {!loadingData && (
              <>
                <span className="ap-badge ap-badge-blue">{isApp ? appFE.total : nbTotal} FE</span>
                {!isApp && <span className="ap-badge ap-badge-orange">{nb8DEnCours} 8D en cours</span>}
                {!isApp && <span className="ap-badge ap-badge-green">{nb8DComplets} 8D complets</span>}
                {!isApp && <span className="ap-badge ap-badge-green">{nbClotures} clôturées</span>}
                {!isApp && <span className="ap-badge ap-badge-orange">{nbEffOui} efficacité OK</span>}
              </>
            )}
          </div>
        </div>
        {tab === "suivi" && (
          <SourceToggle
            source={source} onChange={setSource}
            diapasonLabel="SILOG"
      ...
      <div className="fai-tabs">
        <button className={`fai-tab${tab === "suivi"   ? " active" : ""}`} onClick={() => setTab("suivi")}>📋 Suivi FAI</button>
        <button className={`fai-tab${tab === "blocage" ? " active" : ""}`} onClick={() => setTab("blocage")}>🔒 Blocage DVI</button>
      </div>

      {tab === "suivi" && (
        <>
          <div className="fai-toolbar">
            <select className="ap-select" style={{ minWidth: 80 }} value={annee} onChange={e => { setPage(1); setAnnee(e.target.value); }}>
            ...
            <select className="ap-select" style={{ minWidth: 120 }} value={statut} onChange={e => { setPage(1); setStatut(e.target.value); }}>
              <option value="En cours">En cours</option><option value="Traitée">Traitée</option><option value="Tous">Tous</option>
            </select>
            <input className="ap-input" style={{ flex: 1, minWidth: 180 }} value={q} onChange={e => { setPage(1); setQ(e.target.value); }} placeholder="Recherche N° FE, article…" />
            <button className="ap-btn" onClick={() => { setQ(""); setStatut("En cours"); setAnnee("2026"); setPage(1); }}>Réinitialiser</button>
          </div>
          ...
          <div className="fai-table-wrap">
            <table className="fai-table">
              ...
              <tbody>
                {/* ── Mode App ── */}
                {isApp && rows.map((fe, idx) => (
                  <tr key={fe.id || idx} className="fai-row">
                    <td className="fai-td">
                      <div style={{ fontWeight: 700, color: "#0071e3", fontSize: 13 }}>{fe.numero_fe}</div>
                      <div style={{ fontSize: 12, color: "#6e6e73", marginTop: 2 }}>
                        {fe.date_creation ? new Date(fe.date_creation).toLocaleDateString("fr-FR") : ""}
                      </div>
                    </td>
                    <td className="fai-td"><span className={`ap-badge ${STATUT_BADGE[fe.statut] || "ap-badge-gray"}`}>{fe.statut || "—"}</span></td>
                    <td className="fai-td"><span className="ap-badge ap-badge-blue" style={{ fontSize: 11 }}>{fe.type_evenement || "—"}</span></td>
                    <td className="fai-td" style={{ fontWeight: 600 }}>{fe.quantite || "—"}</td>
                    <td className="fai-td"><span className={`ap-badge ${GRAVITE_BADGE[fe.gravite] || "ap-badge-gray"}`}>{fe.gravite || "—"}</span></td>
                    <td className="fai-td" style={{ fontSize: 12, color: "#6e6e73" }}>{fe.declarant_nom || "—"}</td>
                    <td className="fai-td" style={{ fontSize: 12, color: "#6e6e73", maxWidth: 240, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{fe.description || "—"}</td>
                  </tr>
                ))}

                {/* ── Mode Silog ── */}
                {!isApp && rows.map((fe, idx) => {
                  const ass      = assignments[fe.numero_fe] || { mesure_efficacite: null, cloture: false };
                  ...
                  return (
                    <tr key={fe.numero_fe || idx} className="fai-row">
                      ...
                      <td className="fai-td">
                        <div style={{ fontWeight: 600, fontSize: 12, maxWidth: 150, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={client.name}>{client.name}</div>
                        <div style={{ fontSize: 10.5, color: "#aeaeb2", fontFamily: "monospace", marginTop: 1 }}>{client.code}</div>
                      </td>
                      <td className="fai-td"><span style={{ fontWeight: 700, fontVariantNumeric: "tabular-nums", fontSize: 14 }}>{fe.qte_non_conforme ? Number(fe.qte_non_conforme).toLocaleString("fr-FR") : "—"}</span></td>
                      <td className="fai-td"><span className="ap-badge ap-badge-blue" style={{ fontSize: 11 }}>{fe.dvi_label || fe.type_nc || "—"}</span></td>
                      <td className="fai-td" onClick={e => e.stopPropagation()}>
                        {fe.code_article
                          ? <button className="lct-btn" onClick={() => setLctModal(fe)}>🔍 {(fe.code_article || "").slice(0, 10)}</button>
                      ...
                      <td className="fai-td">
                        <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
                          ...
                          <button className="ap-btn ap-btn-primary" style={{ padding: "3px 9px", fontSize: 11, flexShrink: 0 }} onClick={() => open8D(feEnrichi)}>8D</button>
                        </div>
                      </td>
                      <td className="fai-td" onClick={e => e.stopPropagation()}>
                        <div style={{ display: "flex", gap: 5 }}>
                          <button className={`eff-toggle ${ass.mesure_efficacite === true ? "oui" : "pending"}`} onClick={() => setAss(fe.numero_fe, "mesure_efficacite", ass.mesure_efficacite === true ? null : true)}>
                            {ass.mesure_efficacite === true ? "✓ Efficace" : "Efficace ?"}
                          </button>
                          {ass.mesure_efficacite !== null && (
                            <button className={`eff-toggle ${ass.mesure_efficacite === false ? "non" : "pending"}`} style={{ padding: "5px 8px" }} onClick={() => setAss(fe.numero_fe, "mesure_efficacite", ass.mesure_efficacite === false ? null : false)}>✗</button>
                          )}
                        </div>
                      </td>
                      <td className="fai-td" onClick={e => e.stopPropagation()}>
                        {clotured
                          ? <button className="cloture-btn done">✓ Clôturée</button>
                          : complete
                            ? <button className="cloture-btn ready" onClick={() => handleCloture(fe)}>Clôturer</button>
                            : <button className="cloture-btn blocked" title="8D incomplet — D8 requis">8D incomplet</button>}
                      </td>
                    </tr>
                  );
                })}
                ...
                {!rows.length && !loading && (
                  <tr><td className="fai-td" colSpan={8} style={{ textAlign: "center", padding: 48, color: "#aeaeb2" }}>
          ...
          {!isApp && (
            <div className="ap-pagination">
              ...
              <span style={{ color: "#6e6e73" }}>Page <b>{page}</b> / {totalPages}<span style={{ color: "#aeaeb2", marginLeft: 8 }}>({items.length} FE)</span></span>
      ...
      {lctModal && <LancementModal fe={lctModal} onClose={() => setLctModal(null)} />}

      {confirmCloture && (
        <div className="fai-confirm" onMouseDown={() => setConfirmCloture(null)}>
          <div className="fai-confirm-box" onMouseDown={e => e.stopPropagation()}>
            <div style={{ fontSize: 17, fontWeight: 700, marginBottom: 8 }}>Confirmer la clôture</div>
            <div style={{ fontSize: 13, color: "#6e6e73", marginBottom: 20, lineHeight: 1.6 }}>
              La FE <b style={{ color: "#1d1d1f" }}>{confirmCloture}</b> va être marquée comme <b>Traitée</b>. Cette action est définitive.
            </div>
            <div style={{ display: "flex", justifyContent: "flex-end", gap: 10 }}>
              <button className="ap-btn" onClick={() => setConfirmCloture(null)}>Annuler</button>
              <button className="ap-btn ap-btn-dark" onClick={confirmAndClose}>✓ Confirmer</button>
            </div>
          </div>
        </div>
      )}
